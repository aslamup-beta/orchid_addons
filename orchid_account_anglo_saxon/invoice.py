...
class account_invoice_line(osv.osv):
    ...
    def _anglo_saxon_sale_move_lines(self, cr, uid, i_line, res, context=None):
        ...
        if i_line.product_id.type != 'service' and i_line.product_id.valuation == 'real_time' and inv.od_costing:
            ...
            if dacc and cacc:
                #override by orchid the default goods account to use for the intercompany sales
                cacc = inv and inv.od_inter_exp_acc_id and inv.od_inter_exp_acc_id.id or cacc
...
#                    price = i_line.move_id.product_id.standard_price
                    price = i_line.move_id.price_unit #Added by orchid to use the correct cost as the time of order conformation
                ...
                return [
                    {
                        ...
                        'account_analytic_id': account_analytic_id, #Orchid Added the Analytic
...
#                        'account_analytic_id': False,
    ...
    def _anglo_saxon_purchase_move_lines(self, cr, uid, i_line, res, context=None):
        ...
        inv = i_line.invoice_id
        company_currency = inv.company_id.currency_id.id
        if i_line.product_id and i_line.product_id.valuation == 'real_time' and inv.od_costing:
...
class account_invoice(osv.osv):
    ...
    _columns = {
                'od_costing':fields.boolean('Real Time Costing',readonly=True,states={'draft':[('readonly',False)]})
                }
    _defaults ={
                'od_costing':False
                }
