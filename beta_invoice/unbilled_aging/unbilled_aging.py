...
class BetaUnbilledAgingWiz(models.TransientModel):
    _name = 'od.beta.unbilled.aging.wiz'

    partner_ids = fields.Many2many('res.partner', string="Customer", domain=[('customer', '=', True)])
    account_ids = fields.Many2many('account.account', string="Account", domain=[('id', 'in', [6722, 2123])], default=lambda self: [(6, 0, [6722, 2123])])
    ...
    wiz_line = fields.One2many('od.beta.unbilled.aging.data', 'wiz_id', string="Wiz Line")
    ...
    def _get_lines(self):
        ...
        ctx = {}
        ctx.update(
            {'partner_ids': partner_ids, 'account_ids': account_ids, 'fiscalyear': False, 'all_fiscalyear': True,
             'state': 'posted'})
        cr = self.env.cr
        uid = self.env.uid
        ...
        cr.execute('SELECT DISTINCT res_partner.id AS id,\
                ...
                WHERE (l.account_id=account_account.id) \
                    ...
                    AND (l.date <= %s)\
                    AND ' + self.query + ' \
                ORDER BY res_partner.name', (tuple(move_state), tuple(account_ids), self.date_from, self.date_from,))
        ...
        r_query = 'SELECT l.partner_id, SUM(l.debit-l.credit) \
                    ...
                    AND account_account.active\
                    AND (l.date <= %s)\
                    GROUP BY l.partner_id ', (tuple(move_state), tuple(account_ids), tuple(partner_ids),
        ...
        cr.execute('SELECT l.partner_id, SUM(l.debit-l.credit) \
                   ...
                   (tuple(move_state), tuple(account_ids), tuple(partner_ids), self.date_from, self.date_from,))
        t = cr.fetchall()
        for i in t:
            totals[i[0]] = i[1]
        ...
        if self.direction_selection == 'future':
            cr.execute('SELECT l.partner_id, SUM(l.debit-l.credit) \
                    ...
                    AND (l.date <= %s)\
                        GROUP BY l.partner_id',
                       (tuple(move_state), tuple(account_ids), self.date_from, tuple(partner_ids), self.date_from,
                        self.date_from,))
            t = cr.fetchall()
            for i in t:
                future_past[i[0]] = i[1]
        ...
        history = []
        for i in range(7):
            args_list = (tuple(move_state), tuple(account_ids), tuple(partner_ids), self.date_from,)
            ...
            for partner_info in partners_partial:
                if partner_info[2]:
                    ...
                    cr.execute(
                        '''SELECT MIN(COALESCE(date_maturity,date)) FROM account_move_line WHERE reconcile_partial_id = %s''',
                        (partner_info[2],))
                    date = cr.fetchall()
                    partial = False
    ...
    def od_get_move_ids(self, partner_ids, recv_account_id):
        ...
        qr = qr1 + qr2
        params = (partner_ids, date, date, tuple(account_ids))
        self.env.cr.execute(qr, params)
    ...
    def od_get_open_analytic_ids(self, company_id, partner_id):
        open_an_qr = "select id from account_analytic_account where state not in ('close','cancelled') and company_id=%s and partner_id=%s" % (
    ...
    def print_directly(self):
        ...
        rpt_temp = 'report.od_unbilled_aging_rpt'
        rpt_pool = self.env['od.beta.unbilled.aging.data.main']
        for val in values:
            period_length = 30
            result.append((0, 0, {
                #'wiz_id': wiz_id,
    ...
    def export_rpt(self):
        ...
        period_length = self.period_length
        vw = 'od_unbilled_aging_data_tree_view_1'
        if period_length == 60:
            vw = 'od_unbilled_aging_data_tree_view_2'
        ...
        return {
            ...
            'res_model': 'od.beta.unbilled.aging.data',
            'type': 'ir.actions.act_window',
        }

class unbilled_rpt_data_main(models.TransientModel):
    _name = 'od.beta.unbilled.aging.data.main'

    line_ids = fields.One2many('od.beta.unbilled.aging.data', 'wiz_id_main', string="Wiz Main Line", readonly=True)
...
class wiz_unbilled_rpt_data(models.TransientModel):
    _name = 'od.beta.unbilled.aging.data'

    wiz_id = fields.Many2one('od.beta.unbilled.aging.wiz', string="Wizard")
    wiz_id_main = fields.Many2one('od.beta.unbilled.aging.data.main', string="Wizard")
