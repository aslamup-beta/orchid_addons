...
class opp_rev_sale_in_wiz_new(models.TransientModel):
    _name = 'opp.sale.in.rpt.wiz.new'
...
#     product_group_ids = fields.Many2many('od.product.group',string="Technology Unit",domain=[('code','in',('1','2','3','4'))],default=get_product_grp)
    ...
    sm_ids = fields.Many2many('res.users','wiz_sale_am_id','wiz_id','user_id',string="Customer AM")
    lead_am_ids = fields.Many2many('res.users','wiz_lead_am_id','wiz_id','user_id',string="Lead AM", default = get_lead_am_ids)

    cust_ids = fields.Many2many('res.partner','wiz_customer_id','wiz_id','user_id',domain=[('is_company','=',True),('customer','=',True)],string="Customer")
    owner_ids = fields.Many2many('res.users','wiz_owner_id','wiz_id','user_id',string="Owner")
    sale_team_ids = fields.Many2many('crm.case.section',string="Sales Team")
    presale_ids = fields.Many2many('res.users','wiz_pres_sale_id','wiz_id','user_id',string="Pre-Sale Engineer")
    solution_arc_ids = fields.Many2many('res.users', 'wiz_solution_arc_id', 'rpt_wiz_id', 'user_id',
                                        string="Solution Architect")
    costsheet_id = fields.Many2one('od.cost.sheet', string="Cost Sheet")
    
    wiz_line = fields.One2many('wiz.sale.in.data.new','wiz_id',string="Wiz Line")
...
#             self.lead_am_ids = [(6,0,[])]
    ...
    def export_rpt(self):
        ...
        solution_arc_ids = [sr.id for sr in self.solution_arc_ids]
        fm_view = "od_rev_sale_in_tree_view_1_new"
        if product_group_ids or solution_arc_ids:
            fm_view = "od_rev_sale_in_tree_view_new"
...
#                 lead_am_ids = [user_id, 2446, 2511, 2507, 2583]
                ...
                for line in sheet.summary_weight_line:
                    ...
                    if product_group_ids:
                        ...
                        if line.pdt_grp_id.id in product_group_ids:
                            ...
                            if line.presales_id.id in solution_arc_ids:
                                print ("444444444444444444")
                                prdct_grp_id = line.pdt_grp_id.id
                                solution_arc_id = line.presales_id.id
                                org_pdt_sale = self.env['od.cost.original.summary.group.weight'].search([('cost_sheet_id','=',sheet_id),('pdt_grp_id','=',prdct_grp_id),('presales_id','=',solution_arc_id)],limit=1)
                                if org_pdt_sale:
                                ...
                                result.append((0,0,{
                                    ...
                                    'solution_arc_id': line.presales_id and line.presales_id.id,
                                    'rebate_amnt':rebate
                                    }))
                            else:
                                prdct_grp_id = line.pdt_grp_id.id
                                org_pdt_sale = self.env['od.cost.original.summary.group.weight'].search(
                                    [('cost_sheet_id', '=', sheet_id), ('pdt_grp_id', '=', prdct_grp_id)], limit=1)
                                if org_pdt_sale:
                                    original_gp = org_pdt_sale.total_gp
                                else:
                                    original_gp = 0.0
                        ...
                        if line.presales_id.id in solution_arc_ids:
                            ...
                            prdct_grp_id = line.pdt_grp_id.id
                            solution_arc_id = line.presales_id.id
                            print("line.presales_id", line.presales_id)
                            org_pdt_sale = self.env['od.cost.original.summary.group.weight'].search(
                                [('cost_sheet_id', '=', sheet_id),('presales_id', '=', solution_arc_id)], limit=1)
                            if org_pdt_sale:
                                original_gp = org_pdt_sale.total_gp
                            else:
                                original_gp = 0.0
        ...
        tree_view = model_data.get_object_reference( 'beta_technology_report', fm_view)
        return {
            ...
            'res_model': 'wiz.sale.in.data.new',
...
class wiz_sale_in_rpt_new(models.TransientModel):
    _name = 'wiz.sale.in.data.new'
    wiz_id = fields.Many2one('opp.sale.in.rpt.wiz.new',string="Wizard")
    ...
    solution_arc_id =fields.Many2one('res.users',string="Solution Architect")
