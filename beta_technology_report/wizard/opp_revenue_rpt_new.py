...
class opp_rev_rpt_wiz_new(models.TransientModel):
    _name = 'opp.rev.rpt.wiz.new'
...
#     stage = fields.Selection([(1,'Approved'),(4,'Design Ready'),(12,'Pipeline'),(5,'Commit'),(6,'Lost'),(8,'Cancelled')],string="Opp Stage")
    ...
    wiz_line = fields.One2many('wiz.rev.new.rpt.data','wiz_id',string="Wiz Line")

    presale_ids = fields.Many2many('res.users','wiz_pres_sale_opp_id','wiz_id','user_id',string="Pre-Sale Engineer")
    solution_arc_ids = fields.Many2many('res.users', 'pip_wiz_solution_arc_id', 'pip_rpt_wiz_id', 'user_id',
                                        string="Solution Architect")
    sm_ids = fields.Many2many('res.users','wiz_sale_account_id','wiz_id','user_id',string="Customer AM")
    lead_am_ids = fields.Many2many('res.users','wiz_lead_id','wiz_id','user_id',string="Lead AM", default = get_lead_am_ids)
    cust_ids = fields.Many2many('res.partner','wiz_sale_customer_id','wiz_id','user_id',domain=[('is_company','=',True),('customer','=',True)],string="Customer")
    owner_ids = fields.Many2many('res.users','wiz_sale_owner_id','wiz_id','user_id',string="Owner")
...
#         product_group_id = self.product_group_id and self.product_group_id.id or False
        ...
        if product_group_ids or solution_arc_ids:
            fm_view = "od_rev_opp_tree_view_new"
...
#             domain2 += [('od_partner_class', 'in',cust_class)]
        ...
        return {
            ...
            'res_model': 'wiz.rev.new.rpt.data',
...
class wiz_rev_new_rpt(models.TransientModel):
    _name = 'wiz.rev.new.rpt.data'
    wiz_id = fields.Many2one('opp.rev.rpt.wiz.new',string="Wizard")
