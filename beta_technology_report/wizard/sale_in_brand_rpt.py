...
class opp_rev_sale_in_brand_wiz_extend(models.TransientModel):
    _name = 'opp.sale.in.brand.rpt.wiz.ext'
    ...
    sm_ids = fields.Many2many('res.users', 'ext_wiz_sale_brand_x', 'wiz_id', 'user_id', string="Customer AM")
    lead_am_ids = fields.Many2many('res.users', 'ext_wiz_lead_a2', 'wiz_id', 'user_id', string="Lead AM",
                                   default=get_lead_am_ids)
    cust_ids = fields.Many2many('res.partner', 'ext_wiz_sale_cust1', 'wiz_id', 'user_id',
                                domain=[('is_company', '=', True), ('customer', '=', True)], string="Customer")
    owner_ids = fields.Many2many('res.users', 'ext_wiz_sale_brand_y', 'wiz_id', 'user_id', string="Owner")
    sale_team_ids = fields.Many2many('crm.case.section', string="Sales Team")

    wiz_line = fields.One2many('wiz.sale.in.brand.data.ext', 'wiz_ext_id', string="Wiz Line")
    ...
    def od_load_brands(self):
        ...
        return {
            ...
            'view_type': 'form',
            'view_mode': 'form',
            'res_model': 'opp.sale.in.brand.rpt.wiz.ext',
            'res_id': self.id,
            'view_id': False,
    ...
    def export_rpt(self):
        ...
        fm_view = "od_rev_sale_in_tree_view_1_ext"
        ...
        domain = [('status', '=', 'active'), ('state', 'in', ('approved', 'done', 'modify', 'change', 'analytic_change',
                                                              'change_processed', 'redistribution_processed'))]
        domain2 = []
        ...
        for sheet in cost_sheet_data:
            ...
            if brand_ids:
                for line in sheet.all_brand_weight_line:
                    if line.manufacture_id.id in brand_ids:
                        result.append((0, 0, {
                            'wiz_ext_id': wiz_ext_id,
        ...
        return {
            'domain': [('wiz_ext_id', '=', wiz_ext_id)],
            'name': 'Brand Sale In Report',
            'view_type': 'form',
            'view_mode': 'tree',
            # 'views': [(tree_view and tree_view[1] or False, 'tree')],
            'res_model': 'wiz.sale.in.brand.data.ext',
            'type': 'ir.actions.act_window',
        }


class wiz_sale_in_brand_rpt_ext(models.TransientModel):
    _name = 'wiz.sale.in.brand.data.ext'

    wiz_ext_id = fields.Many2one('opp.sale.in.brand.rpt.wiz.ext', string="Wizard")
    ...
    po_status = fields.Selection(
        [('waiting_po', 'Waiting P.O'), ('special_approval', 'Special Approval From GM'), ('available', 'Available'),
         ('credit', 'Customer Credit')], 'Customer PO Status')
    day_rule = fields.Selection([('ok', 'OK'), ('not_ok', 'Not OK'), ('not_available', 'Not Available')],
                                string="3-Day Rule")
