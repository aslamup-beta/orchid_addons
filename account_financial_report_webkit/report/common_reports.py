...
class CommonReportHeaderWebkit(common_report_header):
    ...
    def _compute_init_balance_employee_custom(self, account_id=None,partner_id=None, period_ids=None, mode='computed', default_values=False):
        if not isinstance(period_ids, list):
            period_ids = [period_ids]
        res = {}
        ...
        if not default_values:
            ...
            try:
                self.cursor.execute("SELECT sum(debit) AS debit, "
                                    ...
                                    " FROM account_move_line"
                                    " WHERE period_id in %s"
                                    " AND account_id = %s AND partner_id=%s", (tuple(period_ids), account_id,partner_id))
                res = self.cursor.dictfetchone()
    ...
    def _compute_init_balance_employee_custom_multi(self, account_id=None,partner_id=None, period_ids=None, mode='computed', default_values=False):
        if not isinstance(period_ids, list):
            period_ids = [period_ids]
        res = {}
        ...
        if not default_values:
            ...
            try:
                self.cursor.execute("SELECT sum(debit) AS debit, "
                                    ...
                                    " FROM account_move_line"
                                    " WHERE period_id in %s"
                                    " AND account_id = %s AND partner_id in %s", (tuple(period_ids), account_id,tuple(partner_id)))
                res = self.cursor.dictfetchone()
    ...
    def _read_opening_balance_employee_custom(self, account_ids,partner_id, start_period,multi):
        """ Read opening balances from the opening balance for employee beta
        """
        opening_period_selected = self.get_included_opening_period(start_period)
        if not opening_period_selected:
            raise osv.except_osv(
        ...
        if multi:
            for account_id in account_ids:
                res[account_id] = self._compute_init_balance_employee_custom_multi(account_id,partner_id, opening_period_selected, mode='read')
        else: 
            for account_id in account_ids:
                res[account_id] = self._compute_init_balance_employee_custom(account_id,partner_id, opening_period_selected, mode='read')
        return res
...
FROM account_move_line l
    ...
    LEFT JOIN account_analytic_account an on (an.id=l.analytic_account_id) 
