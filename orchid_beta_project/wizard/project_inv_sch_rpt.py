...
class project_inv_sch_wiz(models.TransientModel):
    ...
    pmo_planning_date_from = fields.Date(string="PMO Expected Invoice Date From")
    pmo_planning_date_to = fields.Date(string="PMO Expected Invoice Date To")
    
    finance_date_from = fields.Date(string="Finance Expected Invoice Date From")
    finance_date_to = fields.Date(string="Finance Expected Invoice Date To")
    ...
    pending = fields.Boolean(string="Show Pending Only", default=True)
    open = fields.Boolean(string="Show Open Project Only", default=True)
    ...
    def export_rpt(self):
        ...
        pmo_planning_date_from = self.pmo_planning_date_from
        pmo_planning_date_to = self.pmo_planning_date_to
        
        accept_date_from = self.accept_date_from
        accept_date_to = self.accept_date_to
        
        finance_date_from = self.finance_date_from
        finance_date_to = self.finance_date_to
        
        
        
            
        company_id = self.company_id and self.company_id.id 
        domain = [('od_type_of_project','in',('credit','sup','imp','sup_imp','amc','o_m','cust_trn')), ('fin_approved_date','<=', '03-Mar-20 23:00:00')]
        domain2=[]
        if company_id:
        ...
        an_ac_ids = [an.id for an in an_ids if an.state !='cancelled'] 
        ...
        if pmo_planning_date_from:
            domain2 += [('pmo_date','>=',pmo_planning_date_from)]
        if pmo_planning_date_to:
            domain2 += [('pmo_date','<=',pmo_planning_date_to)]
        ...
        if finance_date_from:
            domain2 += [('finance_date','>=',finance_date_from)]
        
        if finance_date_to:
            domain2 += [('finance_date','<=',finance_date_to)]
            
        if self.pending:
            domain2 += ['|',('invoice_status', '=','draft'),('invoice_id', '=',False)]
            
        sch_data = self.env['od.project.invoice.schedule'].search(domain2)
        amc_data = self.env['od.amc.invoice.schedule'].search(domain2)
        result =[]
        ...
        for data in amc_data:
            ...
            result.append((0,0,{
                                ...
                                'planned_date':data.date,
                                'planned_amount':data.amount,
                                'invoice_id':data.invoice_id and data.invoice_id.id if data.invoice_id.name !='Invoice' else False,
                                'invoice_amount':data.invoice_amount,
                                'invoice_status':data.invoice_status,
                                'date_invoice':data.date_invoice,
                                'cust_date':data.cust_date,
        ...
        domain_a = [('od_type_of_project','=','parent_level0'), ('fin_approved_date','>=', '04-Mar-20 01:00:01')]
        domain2_a=[]
        if company_id:
            domain_a += [('company_id','=',company_id)]
        if partner_ids:
            domain_a += [('partner_id','in',partner_ids)]
       
        if branch_ids:
            domain_a += [('od_branch_id','in',branch_ids)]
        
        
        if pm_ids:
            domain_a += [('od_owner_id','in',pm_ids)]
        
        if analytic_ids:
            domain_a +=[('id','in',analytic_ids)]
        if self.open:
            domain_a += [('state','!=', 'close')]
        
        
        analytic_pool = self.env['account.analytic.account']
        
        an_ids = analytic_pool.search(domain_a)
        an_ac_ids = [an.id for an in an_ids if an.state !='cancelled'] 
        
        
        domain2_a +=[('analytic_id','in',an_ac_ids)]
        
        if planning_date_from:
            domain2_a += [('date','>=',planning_date_from)]
        if planning_date_to:
            domain2_a += [('date','<=',planning_date_to)]
            
        if pmo_planning_date_from:
            domain2_a += [('pmo_date','>=',pmo_planning_date_from)]
        if pmo_planning_date_to:
            domain2_a += [('pmo_date','<=',pmo_planning_date_to)]
        
        if accept_date_from:
            domain2_a += [('cust_date','>=',accept_date_from)]
        
        if accept_date_to:
            domain2_a += [('cust_date','<=',accept_date_to)]
            
        if finance_date_from:
            domain2_a += [('finance_date','>=',finance_date_from)]
        
        if finance_date_to:
            domain2_a += [('finance_date','<=',finance_date_to)]
            
        if self.pending:
            domain2_a += ['|',('invoice_status', '=','draft'),('invoice_id', '=',False)]
            
        sch_data = self.env['od.project.invoice.schedule'].search(domain2_a)
        #amc_data = self.env['od.amc.invoice.schedule'].search(domain2)
        for data in sch_data:
            an_id = data.analytic_id and data.analytic_id.id 
            pm_id = data.analytic_id and data.analytic_id.od_owner_id and data.analytic_id.od_owner_id.id
...
class wiz_project_rpt_data(models.TransientModel):
    ...
    pmo_date = fields.Date(string="PMO Expected Date")
