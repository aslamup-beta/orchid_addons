...
class AccountInvoice(models.Model):
    ...
    debit_invoice_id = fields.Many2one(comodel_name='account.invoice', string='Debit Note',
                                       help="Reference to the origin invoice that create this debit note")
    debit_invoice_ids = fields.One2many(comodel_name='account.invoice', inverse_name='debit_invoice_id',
                                        string='Debit Notes', readonly=True,
    ...
    def _prepare_debitnote(self, invoice, date_invoice=None, date=None, description=None, journal_id=None):
        ...
        for field in ['name', 'reference', 'comment', 'date_due', 'partner_id', 'company_id', 'account_id',
                      'currency_id', 'payment_term', 'user_id', 'fiscal_position', 'od_analytic_account', 'od_inter_inc_acc_id', 'od_inter_exp_acc_id']:
            if invoice._fields[field].type == 'many2one':
                values[field] = invoice[field].id
            else:
                values[field] = invoice[field] or False

        values['invoice_line'] = self._debitnote_cleanup_lines((invoice.invoice_line))
        ...
        if journal_id:
            ...
            debitnote_journal = self.env['account.journal'].search([('type', '=', 'purchase')], limit=1)
        else:
            debitnote_journal = self.env['account.journal'].search([('type', '=', 'sale')], limit=1)
    ...
    @api.multi
    @api.returns('self')
    def debitnote(self, date_invoice=None, date=None, description=None, journal_id=None):
        new_invoice = []
        for invoice in self:
