...
class BetaTerminateForm(models.Model):
    _name = 'od.beta.terminate.form'
    _inherit = ['mail.thread', 'ir.needaction_mixin']
    _description = "Beta Termination Form"
...
#             monthly_salary = contract_ob.xo_total_wage
    ...
    # state = fields.Selection([('draft', 'Start'),('approval1', 'Direct Manager'), ('approval4', 'IT Approval'), ('notify_bank', 'Waiting to Notify Bank'), ('approval2', 'Finance Approval'),('approval3', 'GM Approval'),('approval5', 'HR Approval'),('confirm', 'Confirmed'), ('sent_clearance', 'Clearance Send to Bank'), ('cancel', 'Refused')],
    ...
    term_date = fields.Date(string='Termination Date',copy=False)
    ...
    def onchange_term_date(self):
        if self.term_date and self.company_id.id == 1:
            term_date = datetime.strptime(self.term_date, "%Y-%m-%d")
            last_day_date =  term_date + relativedelta(months=3)
            self.last_day_date = last_day_date
        if self.term_date and self.company_id.id == 6:
            term_date = datetime.strptime(self.term_date, "%Y-%m-%d")
            last_day_date =  term_date + relativedelta(months=2)
            self.last_day_date = last_day_date
    ...
    def create(self,vals):
        res = super(BetaTerminateForm, self).create(vals)
        res.submit()
        return res
    ...
    def finance_approval(self):
        ...
        self.od_send_mail('od_termination_approval2')
    ...
    def action_send_it_approval_mail(self):
        ...
        self.od_send_mail('od_termination_approval_it')
        return True
    ...
    def action_notify_bank(self):
        if self.employee_id.od_req_clearance and self.company_id.id == 6:
            self.od_send_mail('od_notify_bank_termination')
            self.state = 'approval2'
    ...
    def second_approval(self):
        self.od_send_mail('od_terminate_notification_to_hr')
    ...
    def hr_approval(self):
        ...
        self.employee_id.write({'od_terminated':True, 'od_last_day':self.last_day_date})
        if self.company_id.id == 6:
            self.state = 'approval4'
        else:
            self.state = 'confirm'
    ...
    def sent_clearance_to_bank(self, cr, uid, ids, context=None):
        ...
        for rec in self.browse(cr, uid, ids, context=context):
            if rec.notified_bank and rec.company_id.id == 6:
                ...
                template_id = ir_model_data.get_object_reference(cr, uid, 'beta_customisation', 'od_sent_clearance_to_bank_termination_saudi')[1]
                try:
                ...
                ctx = dict(context)
                ctx.update({
                'default_model': 'od.beta.terminate.form',
    ...
    def unlink(self):
        ...
        return super(BetaTerminateForm, self).unlink()
...
class mail_compose_message(models.Model):
    ...
    def send_mail(self):
        ...
        if context.get('default_model') == 'od.beta.terminate.form' and \
                context.get('default_res_id') and context.get('bank_clearance_sent'):
            rec = self.env['od.beta.terminate.form'].browse(context['default_res_id'])
