...
class BetaResignForm(models.Model):
    ...
    def _check_user(self):
        ...
        if self.employee_id and self.resign_date and self.company_id.id == 1 and current_user not in (268,2131,12,2224):
            resign_date = datetime.strptime(self.resign_date, "%Y-%m-%d")
            original_last_day_date =  resign_date + relativedelta(months=3)
            current_last_day_date = datetime.strptime(self.last_day_date, "%Y-%m-%d")
        ...
        if self.company_id.id == 6:
            if self.create_date < '2025-08-01' :
                ...
                if last_day:
                    ...
                    days = 0.0
                    if service < 1.0:
                        leaves = self.env['hr.holidays'].search([('holiday_status_id','=',1),('employee_id','=',employee_id),('state','not in',('draft', 'cancel','refuse'))])
                        for obj in leaves:
                            days += obj.od_number_of_days
                            leave_taken = days
    ...
    def _get_pending_salary(self):
        ...
        if has_pending_salary and start_date and end_date:
            ...
            total_days_in_same_month = self.days_in_same_month(end_date)
            if self.company_id.id == 6:
                if days ==31:
                    days = 30
    ...
    def _compute_unpaid_leaves(self):
        ...
        if last_day:
            ...
            employee_id = self.employee_id and self.employee_id.id
            days = 0.0
            leaves = self.env['hr.holidays'].search([('od_start','>',joining_date),('od_end','<',last_day),('holiday_status_id','=',4),('employee_id','=',employee_id),('state','not in',('draft', 'cancel','refuse'))])
            for obj in leaves:
                days += obj.od_number_of_days
            if days:
                self.unpaid_days = days

    def compute_unpaid_leaves(self, emp_id):
    ...
    def _compute_total_days(self, emp_id):
        print("_compute_total_days")
        emp_id = self.employee_id or False
        unpaid_days = self.compute_unpaid_leaves(emp_id)
...
#             monthly_salary = contract_ob.xo_total_wage
    ...
    def check_six_months_conditions(self):
        ...
        last_day = self.last_day_date or False
        mrg_date = employee.od_mrg_date or False
        if mrg_date and last_day:
            days = self.days_between(mrg_date, last_day)
            return days < 181
        else:
            return False
    ...
    def check_three_months_conditions(self):
        employee = self.employee_id
        last_day = self.last_day_date or False
        child_birth_date = employee.od_child_birth_date or False
        if child_birth_date and last_day:
            days = self.days_between(child_birth_date, last_day)
            return days < 90
        else:
            return False
    ...
    def _get_amount_to_pay(self):
        ...
        if self.company_id.id == 6:
            if service < 2.0:
                self.amount_to_pay = 0 + remaining_amt
                self.final_grat = 0
            if  2<= service <=5:
                self.amount_to_pay = gratuity/3.0 + remaining_amt
                self.final_grat = gratuity/3.0
            if  5< service <10:
                self.amount_to_pay = gratuity*(2.0/3.0) + remaining_amt
                self.final_grat = gratuity*(2.0/3.0)
            if service >= 10.0:
                self.amount_to_pay = gratuity + remaining_amt
                self.final_grat = gratuity
            if self.force_majure :
                self.amount_to_pay = gratuity + remaining_amt
                self.final_grat = gratuity
    ...
    manager1_id = fields.Many2one('res.users', string='Direct Manager')
    ...
    bank_clearnce = fields.Selection([('yes', 'YES'),('no', 'NO')],string="Bank Clearance Required?")
    ...
    fin_approved_by = fields.Many2one('res.users', string='CFO')
    ...
    cs_feedback_1 = fields.Selection([('confirmed', 'Confirmed'),('not_confirmed', 'Not Confirmed')],string="CS Feedback 1")
    cs_feedback_2 = fields.Selection([('confirmed', 'Confirmed'),('not_confirmed', 'Not Confirmed')],string="CS Feedback 2")
    cs_feedback_3 = fields.Selection([('confirmed', 'Confirmed'),('not_confirmed', 'Not Confirmed')],string="CS Feedback 3")
    cs_feedback_4 = fields.Selection([('confirmed', 'Confirmed'),('not_confirmed', 'Not Confirmed')],string="CS Feedback 4")
    cs_feedback_5 = fields.Selection([('confirmed', 'Confirmed'),('not_confirmed', 'Not Confirmed')],string="CS Feedback 5")
    ...
    def onchange_employee_id(self):
        ...
        #company_id = employee.company_id and employee.company_id.id
        if employee:
    ...
    def onchange_resign_date(self):
        if self.resign_date and self.company_id.id == 1:
            resign_date = datetime.strptime(self.resign_date, "%Y-%m-%d")
            last_day_date =  resign_date + relativedelta(months=3)
            self.last_day_date = last_day_date
        if self.resign_date and self.company_id.id == 6:
            resign_date = datetime.strptime(self.resign_date, "%Y-%m-%d")
    ...
    def first_approval(self):
        if self.env.user.id == self.employee_id.user_id.id:
            raise Warning("You Cannot Approve your own Resignation, Please Ask your Manager to do it..")
    ...
    def finance_approval(self):
        if self.env.user.id == self.employee_id.user_id.id:
            raise Warning("You Cannot Approve your own Resignation, Please Ask Finance Team to do it..")
        if self.employee_id.od_req_clearance:
    ...
    def action_send_it_approval_mail(self):
        ...
        self.od_send_mail('od_resignation_approval_it')
        return True
    ...
    def action_notify_bank(self):
        if self.employee_id.od_req_clearance and self.company_id.id == 6:
            self.od_send_mail('od_notify_bank_resignation')
            self.state = 'approval2'
    ...
    def hr_approval(self):
        if self.env.user.id == self.employee_id.user_id.id:
            raise Warning("You Cannot Approve your own Resignation, Please Ask your HR Manager to do it..")
        self.employee_id.write({'od_resigned':True, 'od_last_day':self.last_day_date})
        if self.company_id.id == 6:
            self.state = 'approval4'
        else:
            self.state = 'confirm'
    ...
    def second_approval(self):
        if self.env.user.id == self.employee_id.user_id.id:
            raise Warning("You Cannot Approve your own Resignation, Please Ask GM to do it..")
        self.od_send_mail('od_notification_to_hr')
    ...
    def sent_clearance_to_bank(self, cr, uid, ids, context=None):
        ...
        for rec in self.browse(cr, uid, ids, context=context):
            if rec.notified_bank and rec.company_id.id == 6:
                ...
                template_id = ir_model_data.get_object_reference(cr, uid, 'beta_customisation', 'od_sent_clearance_to_bank_saudi')[1]
                try:
...
class mail_compose_message(models.Model):
    ...
    def send_mail(self):
        ...
        if context.get('default_model') == 'od.beta.resign.form' and \
