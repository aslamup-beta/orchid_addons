...
class BetaTenderRequest(models.Model):
    _name = 'od.tender.request'
    _inherit = ['mail.thread', 'ir.needaction_mixin']
    _description = "Tender Request Form"
    _rec_name = 'job_name'
...
#                                   string='Guarantee Name', default="bid_bond")
    ...
    enter_lg_amt = fields.Boolean(string="Enter Tender Amt and Convert Percent?")
    enter_lg_per = fields.Boolean(string="Enter Tender Percent and Convert Amt?")
    lg_amt = fields.Float(string="Tender Amount")
    lg_perc = fields.Float(string="Tender %")
    lg_amt1 = fields.Float(string="Tender Amount")
    lg_perc1 = fields.Float(string="Tender %")
    ...
    def btn_confirm(self):
        if not (self.enter_lg_amt or self.enter_lg_per):
            raise Warning("Please enter Tender amount using one of the two options.")
        self.requested_by = self.env.user.id
        self.req_date = fields.Date.today()
        self.od_send_mail('od_notify_finance_verify_tender_request')
        self.write({'state':'draft'})
    
    @api.model
    def create(self,vals):
        res = super(BetaTenderRequest, self).create(vals)
        res.btn_confirm()
        return res
    
    @api.one
    def btn_refuse(self):
        self.od_send_mail('od_notify_sam_refuse_tender_request')
    ...
    def btn_verify(self):
        ...
        self.od_send_mail('od_notify_gm_approve_tender_request')
    ...
    def btn_approve(self):
        ...
        self.od_send_mail('od_notify_sam_verify_tender_request')
