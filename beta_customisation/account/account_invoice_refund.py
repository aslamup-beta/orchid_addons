    ...
    def compute_refund(self, cr, uid, ids, mode='refund', context=None):
        ...
        for form in self.browse(cr, uid, ids, context=context):
            ...
            for inv in inv_obj.browse(cr, uid, context.get('active_ids'), context=context):
                od_refund = inv.od_refund
                analytic_id = inv.od_analytic_account and inv.od_analytic_account.id or False
                if not od_refund and inv.type == 'out_invoice':
                    raise osv.except_osv(_('Insufficient Data!'), \
                                            _('Please Mark as Refunded/Cancelled in the Invoice Form '))
                ...
                if inv.date_invoice > '2025-07-28':
                    enable = inv.bt_enable
                else:
                    enable = False
                inv_obj.write(cr, uid, [refund.id], {'date_due': date,
                                                ...
                                                'od_allow_extra_exp':True,
                                                'od_original_invoice_id': inv.id,
                                                'bt_enable': enable,
                                                'bt_pay_perc' : inv.bt_pay_perc if inv.bt_enable else 0.0
                                                })
