...
class report_orchid_stock_report_90(report_sxw.rml_parse):
    _name = "beta_customisation.report.orchid.stock.report"

    def set_context(self, objects, data, ids, report_type=None):
        return super(report_orchid_stock_report_90, self).set_context(objects, data, ids, report_type=report_type)


    def __init__(self, cr, uid, name, context):
        super(report_orchid_stock_report_90, self).__init__(cr, uid, name, context=context)
    ...
    def get_single_vals(self,data):
        for val in data:
            res = {'total_qty':0.0,'total_val':0.0,'0-90':0.0,'0-90val':0.0,'90-180':0.0,'90-180val':0.0,
                   '180-270':0.0,'180-270val':0.0,'270-360':0.0,'270-360val':0.0,'360-abv':0.0,'360-abvval':0.0,}
            for line in val.get('lines'):
                res['total_qty'] += line['qty']
                res['total_val'] += line['inventory_value']
                if line['age'] >0 and line['age'] <=90:
                    res['0-90'] += line['qty']
                    res['0-90val'] += line['inventory_value']
                elif line['age'] >90 and line['age'] <=180:
                    res['90-180'] += line['qty']
                    res['90-180val'] += line['inventory_value']
                elif line['age'] >180 and line['age'] <=270:
                    res['180-270'] += line['qty']
                    res['180-270val'] += line['inventory_value']
                elif line['age'] >270 and line['age'] <=360:
                    res['270-360'] += line['qty']
                    res['270-360val'] += line['inventory_value']
                elif line['age']>360:
                    res['360-abv'] += line['qty']
                    res['360-abvval'] += line['inventory_value']
...
class report_stock_aging(osv.AbstractModel):
    _name = 'report.beta_customisation.report_stock_aging_90'
    _inherit = 'report.abstract_report'
    _template = 'beta_customisation.report_stock_aging_90'
    _wrapped_report_class = report_orchid_stock_report_90
