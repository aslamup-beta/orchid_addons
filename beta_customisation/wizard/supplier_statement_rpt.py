...
class supplier_statement(models.TransientModel):
    _name = 'supplier.statement.rpt.wiz'
    ...
    partner_id = fields.Many2one('res.partner',string="Supplier",  default= get_default_partner_id)
    ...
    def get_data(self):
        ...
        invoice_ids = self.env['account.invoice'].search([('state','in',('open', 'accept')),('type','=', 'in_invoice'), ('partner_id','=',partner_id),('od_ignore','=', False),('account_id.type', '=', 'payable'), ('id', '!=', 13713)])
        move_ids = []
        payment_move_ids = []
        result  = []
            ...
            if bal:
                if inv.payment_ids:
                    result.append((0,0,{
                        ...
                        'actual_inv_date':inv.move_id.actual_inv_date or False,
                        'beta_ref':inv.number,
                        'part_ref':inv.supplier_invoice_number or "/",
                        'due':due *-1.0,
                        'paid':paid *-1.0,
                        'bal':bal*-1.0,
                        'age':age,
                        'payment_term_id': inv.payment_term and inv.payment_term.id or False,
                        'remark':inv.od_remarks[:20] if inv.od_remarks else "",
                        'due_date': inv.date_due or False
                        }))
                else:
                    result.append((0,0,{
                        'date':inv.move_id.date,
        ...
        for mvl in due_moves:
            ...
            result.append((0,0,{
                'date':mvl.date,
                'actual_inv_date':mvl.invoice.actual_inv_date or False,
                'beta_ref':mvl.ref,
                'part_ref':mvl.name or "/",
                'due':mvl.credit * -1 ,
                'age':age,
                'paid':mvl.debit* -1,
                'bal': (mvl.credit - mvl.debit)*-1,
                'payment_term_id': mvl.invoice and mvl.invoice.payment_term and mvl.invoice.payment_term.id or False,
                'remark':  mvl.invoice.od_remarks[:20] if mvl.invoice.od_remarks else "",
                'due_date': mvl.invoice.date_due or False
                }))
        return result
    ...
    def get_bucket_vals(self,partner_id):
        ...
        period_length = 30
        due_zero_thirty = reduce(lambda x, y: x + ((y['account_id']['type'] == 'payable' and y['credit'] * -1 or 0) ), self._lines_get_ord(partner_id,period_length,1), 0)
        due_thirty_sixty = reduce(lambda x, y: x + ((y['account_id']['type'] == 'payable' and y['credit'] * -1 or 0) ), self._lines_get_ord(partner_id,period_length,2), 0)
        due_sixty_ninty = reduce(lambda x, y: x + ((y['account_id']['type'] == 'payable' and y['credit'] * -1 or 0) ), self._lines_get_ord(partner_id,period_length,3), 0)
        due_ninty_onetwenty = reduce(lambda x, y: x + ((y['account_id']['type'] == 'payable' and y['credit'] * -1 or 0) ), self._lines_get_ord(partner_id,period_length,4), 0)
        due_onetwenty_onefifty = reduce(lambda x, y: x + ((y['account_id']['type'] == 'payable' and y['credit'] * -1 or 0) ), self._lines_get_ord(partner_id,period_length,5), 0)
        due_onefifty_oneeighty = reduce(lambda x, y: x + ((y['account_id']['type'] == 'payable' and y['credit'] * -1 or 0) ), self._lines_get_ord(partner_id,period_length,6), 0)
        due_oneeighty_plus = reduce(lambda x, y: x + ((y['account_id']['type'] == 'payable' and y['credit'] * -1 or 0) ), self._lines_get_plus(partner_id,period_length,6), 0)
            
        paid_zero_thirty = reduce(lambda x, y: x + ((y['account_id']['type'] == 'payable' and y['debit'] * -1 or 0) ), self._lines_get_ord(partner_id,period_length,1), 0)
        paid_thirty_sixty = reduce(lambda x, y: x + ((y['account_id']['type'] == 'payable' and y['debit'] * -1 or 0) ), self._lines_get_ord(partner_id,period_length,2), 0)
        paid_sixty_ninty = reduce(lambda x, y: x + ((y['account_id']['type'] == 'payable' and y['debit'] * -1 or 0) ), self._lines_get_ord(partner_id,period_length,3), 0)
        paid_ninty_onetwenty = reduce(lambda x, y: x + ((y['account_id']['type'] == 'payable' and y['debit'] * -1 or 0) ), self._lines_get_ord(partner_id,period_length,4), 0)
        paid_onetwenty_onefifty = reduce(lambda x, y: x + ((y['account_id']['type'] == 'payable' and y['debit'] * -1 or 0)), self._lines_get_ord(partner_id,period_length,5), 0)
        paid_onefifty_oneeighty = reduce(lambda x, y: x + ((y['account_id']['type'] == 'payable' and y['debit'] * -1 or 0) ), self._lines_get_ord(partner_id,period_length,6), 0)
        paid_oneeighty_plus = reduce(lambda x, y: x + ((y['account_id']['type'] == 'payable' and y['debit'] * -1 or 0) ), self._lines_get_plus(partner_id,period_length,6), 0)
    ...
    def print_directly(self):
        data = self.get_data()
        partner_id = self.partner_id.id
        rpt_pool = self.env['od.supplier.statement.rpt.data']
        ...
        cr = self.env.cr
        uid = self.env.uid
        return self.pool['report'].get_action(cr, uid, [rpt_id], 'report.od_supplier_statement', context=ctx)
...
#         self.write({'wiz_line':result})
        ...
        return {
            ...
            'view_mode': 'form',
             'res_id':rpt_id,
            'res_model': 'od.supplier.statement.rpt.data',
            'type': 'ir.actions.act_window',
        }
    
class customer_statement_rpt_data(models.TransientModel):
    _name = 'od.supplier.statement.rpt.data'
    ...
    line_ids = fields.One2many('supplier.statement.rpt.data.line','wiz_id',string="Wiz Line",readonly=True)
    ...
    def print_supplier_statement(self, cr, uid, ids, context=None):
        return self.pool['report'].get_action(cr, uid, ids, 'report.od_supplier_statement', context=context)
    
class supplier_statement_rpt_data_line(models.TransientModel):
    _name = 'supplier.statement.rpt.data.line'
    _order = 'date desc'
    
    company_id = fields.Many2one('res.company',string="Company")
    wiz_id = fields.Many2one('od.supplier.statement.rpt.data',string="Wizard")
