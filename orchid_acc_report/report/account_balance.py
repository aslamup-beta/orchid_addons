...
#             qr =qr1+cust_query + qr2
        ...
        def od_get_open_analytic_ids(company_id,partner_id):
        ...
        def get_advance_invoice_account():
                ...
                #ksa 5332
            return [4006,5332]
        def each_get_analytic_amounts(total_amount,partner_id,company_id,recv_account_id):
            print "getting amount"
            
            move_ids = od_get_move_ids(partner_id,recv_account_id)
            print "move_ids>>>>>"
            analytic_ids = od_get_open_analytic_ids(company_id,partner_id)
        ...
        def get_analytic_amounts(company_id,recv_account_id):
                open_amount,closed_amount =0.0,0.0 
                partner_ids = od_get_partner_ids()
                for partner_id in partner_ids:
                    total_amount =od_get_total_rec_amount(partner_id,recv_account_id)
                    op_amt,cl_amt = each_get_analytic_amounts(total_amount,partner_id,company_id,recv_account_id)
                    open_amount += op_amt 
                    closed_amount += cl_amt
        ...
        def _process_child(accounts, disp_acc, parent):
                ...
                branch_id = acc_id.od_branch_id and acc_id.od_branch_id.id or False
...
#                    
        ...
        wiz_branch_id = form.get('od_branch_id') and form.get('od_branch_id')[0] or False
        seen = set()
        new_l = []
        for d in new_res:
            t = tuple(d.items())
            if t not in seen:
                seen.add(t)
                new_l.append(d)
        #Added by aslam to show only selected branch accounts        
        if wiz_branch_id:
            new_l = [d for d in new_l if d['branch_id'] in [wiz_branch_id]]
        return new_l
